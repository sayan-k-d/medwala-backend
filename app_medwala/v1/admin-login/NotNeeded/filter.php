...
if (!isset($_GET['subid'])) {
	...
	$find_artists = find("all", FILTER, '*', "WHERE subcategory = '".$_GET['subid']."'", array());
	$findsubcat = find("first", CATAGORY, '*', "WHERE id = '".$_GET['subid']."' ", array());
	if(isset($_POST['submit']))
	{
		...
		$execute=array(
			':name'=>$_POST['name'],
			':subcategory'=>$_GET['subid'],
			);
		$save_data = save($table, $fields, $values, $execute);
		if($save_data)
		{
			header('location:filter.php?subid='.$_GET['subid']);
		}
		else
		{
	...
	if(isset($_GET['id']))
	{
	    $find_artists = find("all", SUBFILTER, '*', "WHERE filter = '".$_GET['id']."' ", array());
		$id=$_GET['id'];
		$table=FILTER;
		...
		foreach($find_artists as $subfils){
		    $table=SUBFILTER;
							...
							<div class="col-lg-12 mb-12">
										<div class="card">
										  <div class="card-header">
											<h6 class="text-uppercase mb-0">FILTER FOR <?php echo $findsubcat['name'];?> - <button type="button" data-toggle="modal" data-target="#myModal2" class="btn btn-primary">Add New Filter</button></h6>
													  ...
													  <div role="document" class="modal-dialog">
														<div class="modal-content">
														  <div class="modal-header">
															<h4 id="exampleModalLabel" class="modal-title">Add Filter</h4>
											...
											<table id="example" class="table table-striped table-hover card-text">
											  ...
											  ?>
												<tr>
												  ...
												  <td><a href="subfilters.php?filterid=<?php echo $admin['id']; ?>"><button name="edit" type="submit" class="btn btn-primary">Sub Filters</button></a></td>
												  <td><button type="button" data-toggle="modal" data-target="#editmodal_<?php echo $admin['id']; ?>" class="btn btn-primary">Edit</button></td>
													...
													<div id="editmodal_<?php echo $admin['id']; ?>"  role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
													  <div role="document" class="modal-dialog">
														<div class="modal-content">
														  <div class="modal-header">
															<h4 id="exampleModalLabel" class="modal-title">Edit Filter</h4>
															<button type="button" data-dismiss="modal" aria-label="Close" class="close"><span aria-hidden="true">×</span></button>
														  </div>
														  <div class="modal-body">
															<form method="post" enctype="multipart/form-data" class="form-horizontal">
												...
												<!--Edit Modal-->
												  ...
												  <td><a href="filter.php?id=<?php echo $admin['id']; ?>&subid=<?php echo $_GET['subid']; ?>"  onclick="return confirm('Are you sure you want to delete this item?');"><button name="delete" type="submit" class="btn btn-primary">Delete</button></a></td>
												</tr>
											<?php
													}
												}
