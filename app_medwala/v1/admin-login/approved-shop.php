...
session_start();
	...
	if(isset($_GET['id']))
	{
		...
		$applyshop = find("first", APPLYVENDOR, '*', "WHERE id = '$id' ", array());
		$url = $shop['url'];
		
	}
	else
	{
		header("location:appliedshop.php");exit;
	}
	...
	if(isset($_POST['submit']))
	{
	   ...
	   // exit();
			$table=SHOP;
			$fields="name,categories,categories_name,address,profile_image,joindate,jointime,url,name_of_owner,email_of_owner,phone_of_owner,trade_lisence,drug_lisence,shop_address_proof,lati,longi,contact_person_name,contact_person_email,contact_person_phone,description,discount_percentage,area,deliverydistance,status,badgetext,badgetextcolor,badgebackgroundcolor";
			$values=":name,:categories,:categories_name,:address,:profile_image,:joindate,:jointime,:url,:name_of_owner,:email_of_owner,:phone_of_owner,:trade_lisence,:drug_lisence,:shop_address_proof,:lati,:longi,:contact_person_name,:contact_person_email,:contact_person_phone,:description,:discount_percentage,:area,:deliverydistance,:status,:badgetext,:badgetextcolor,:badgebackgroundcolor";
			$execute=array(
				':name'=>$_POST['name'],
			...
			if($save_data)
			{
				...
				if($folders)
				{
            		...
            		$where_clause="WHERE id=".$applyshop['id'];
            		$execute=array(
							...
							<div class="col-lg-12 mb-5">
								<div class="card">
								  <div class="card-header">
									<h3 class="h6 text-uppercase mb-0">APPROVED SHOP</h3>
								  </div>
								  <div class="card-body">
									<form method="post"  enctype="multipart/form-data" class="form-horizontal">
									  ...
									  <div class="form-group row">
										...
										<div class="col-md-9">
										  <input name="name" type="text" class="form-control" value="<?php echo $applyshop['shopname']; ?>" required>
										</div>
									  </div>
									  ...
									  <div class="form-group row">
										...
										<div class="col-md-9">
										  <input name="address" type="text" class="form-control" required value="<?php echo $applyshop['address']; ?>" >
										</div>
									  </div>
									  ...
									  <h4>Ownership Details</h4>
									   ...
									   <div class="form-group row">
										...
										<div class="col-md-9">
										  <input name="name_of_owner" type="text" class="form-control" required value="<?php echo $applyshop['name']; ?>" >
										</div>
									  </div>
									  <div class="line"></div>
									   <div class="form-group row">
										...
										<div class="col-md-9">
										  <input name="email_of_owner" type="email" class="form-control" required value="<?php echo $applyshop['email']; ?>" >
										</div>
									  </div>
									  ...
									  <div class="line"></div>
									   <div class="form-group row">
										...
										<div class="col-md-9">
										  <input name="phone_of_owner" type="text" class="form-control" required pattern="[6-9]{1}[0-9]{9}" title="Only 10 digits phone number allowed" value="<?php echo $applyshop['phone']; ?>" >
										</div>
									  </div>
